...
!-------------------- namelist -----------------------------------------
   ...
   real :: t_zero=315., t_strat=200., delh=60., delv=10.,delvu= 0., eps=0., sigma_b=0.7
   real :: P00 = 1.e5
...
!-----------------------------------------------------------------------
   ...
   namelist /hs_forcing_nml/  no_forcing, t_zero, t_strat, delh, delv, delvu, eps, &
...
!     thermal forcing for held & suarez (1994) benchmark calculation
      ...
      call newtonian_damping ( lon, lat, ps, p_full, t, ttnd, teq, mask )
...
!#######################################################################
 ...
 subroutine newtonian_damping ( lon, lat, ps, p_full, t, tdt, teq, mask )
...
!-----------------------------------------------------------------------
     ...
     tstr, sigma, the, tfactr, rps, p_norm
       ...
       real, dimension(size(t,1),size(t,2),size(t,3)) :: Qzero

       integer :: k, j, i
       real    :: tcoeff, pref, yy, xx, zz, xindex
...
!  ----- compute equilibrium temperature (teq) -----
         ...
         the   (:,:) = t_star(:,:) - (delv*cos_lat_2(:,:) + delvu)*log(p_norm(:,:)) 
...
!  ----- compute Gill -----
      ...
      do i=1,size(t,1)
         do j=1, size(t,2)         
            do k=1, size(t,3)
               yy=5*lat(i,j)/lat(i,size(t,2))
               xindex=ceiling(0.5*size(t,1))
               xx=10*(lon(i,j)-lon(xindex,j))/(lon(size(t,1),j)-lon(xindex,j))
               zz=ceiling(0.4*size(t,3))
               if (abs(xx)>2) then
                  Qzero(i,j,k)=0
               else if (k<zz) then
                  Qzero(i,j,k)=0
               else
                  Qzero(i,j,k)=4*0.00001*cos(xx*pi/4)*exp(0-yy*yy/4)*sin(pi*(p_full(i,j,k)-p_full(i,j,zz))/(p_full(i,j,size(t,3))-p_full(i,j,zz)))
               endif
            enddo
         enddo
      enddo
